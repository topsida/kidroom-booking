...
function buildHtml(rooms: Room[]): string {
  const pts = rooms
...
export function RoomsMap({ rooms }: { rooms: Room[] }) {
  ...
  const html = useMemo(() => buildHtml(rooms), [rooms]);

  function onMessage(event: WebViewMessageEvent) {
    try {
      const data = JSON.parse(event.nativeEvent.data);
      if (data.type === 'select') setSelected(rooms.find(r => r.id === data.id) ?? null);
      else if (data.type === 'deselect') setSelected(null);
    } catch {}
  }
  ...
  return (
    <View style={styles.container}>
      <WebView
        style={styles.map}
        source={{ html, baseUrl: 'https://carto.com' }}
        onMessage={onMessage}
        originWhitelist={['*']}
        javaScriptEnabled
        domStorageEnabled
      ...
      <Modal visible={!!picker} transparent animationType="fade" onRequestClose={() => setPicker(null)}>
        ...
        <View style={styles.dialogWrap} pointerEvents="box-none">
          <View style={styles.dialog}>
            ...
            <TouchableOpacity style={styles.dialogRow} onPress={() => picker && launchYandex(picker)} activeOpacity={0.7}>
              <Image source={YANDEX_ICON} style={styles.appIcon} />
              <Text style={styles.dialogRowText}>Яндекс Карты</Text>
              <Ionicons name="chevron-forward" size={18} color={C.textLight} />
            </TouchableOpacity>
            <View style={styles.dialogDivider} />
            <TouchableOpacity style={styles.dialogRow} onPress={() => picker && launchGoogle(picker)} activeOpacity={0.7}>
              <Image source={GOOGLE_ICON} style={styles.appIcon} />
              <Text style={styles.dialogRowText}>Google Maps</Text>
              <Ionicons name="chevron-forward" size={18} color={C.textLight} />
            </TouchableOpacity>
            <View style={styles.dialogDivider} />
            <TouchableOpacity style={[styles.dialogRow, styles.dialogCancel]} onPress={() => setPicker(null)} activeOpacity={0.7}>
...
function makeStyles(C: ThemeColors) {
  return StyleSheet.create({
    ...
    dialogSubtitle: {
      ...
      paddingBottom: 16, paddingHorizontal: 20, marginTop: 4,
    },
    dialogDivider:    { height: StyleSheet.hairlineWidth, backgroundColor: C.border },
    dialogRow:        { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 16, gap: 14 },
    appIcon:          { width: 24, height: 24, borderRadius: 6 },
    dialogRowText:    { flex: 1, fontSize: 16, color: C.text, fontWeight: '500' },
    dialogCancel:     { justifyContent: 'center' },
    dialogCancelText: { flex: 1, fontSize: 16, color: C.error, fontWeight: '600', textAlign: 'center' },
  });
}
